...
const directions = document.querySelector('.inst');
...
function hit(e){
    ...
    showMessage('You took ' + duration + ' seconds to click');
    if (duration > maxDuration){
        gameArea.children[0].remove();
        results.innerHTML = `Too Slow! <span id="loser">You Lose!</span> Your score is ${count}.`;
        resetGame();
    } else {
        gameArea.children[0].remove();
        playArea.timer = setTimeout(myShape, random(4000));
